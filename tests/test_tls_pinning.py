...
def test_dev_mode_still_works_as_an_escape_hatch(certs, monkeypatch):
...
def test_pinning_takes_precedence_over_dev_mode(certs, monkeypatch):
    """A pin is a stronger statement than 'trust anything' — it must win."""
    cert_path, key_path = certs
    monkeypatch.setenv("AOS8_DEV_MODE", "true")
    with _TLSServer(cert_path, key_path) as srv:
        good = fetch_cert_fingerprint("127.0.0.1", port=srv.port, timeout=5)
        client = _client(srv.port, monkeypatch, pin_fingerprint=good)
        assert client.pinned_fingerprint == normalize_fingerprint(good)
        assert client.connect() is True
