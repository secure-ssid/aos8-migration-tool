...
| Port 4343 firewalled or REST API disabled | The REST API is on TCP **4343**. Confirm reachability from the machine running the tool; otherwise switch to **Paste CLI output** mode. |
| TLS / certificate error on connect | Certificate verification is **ON by default** — the login carries controller admin credentials. A stock factory self-signed cert chains to no CA, so Step 1 stops and shows the certificate's SHA-256 fingerprint: confirm it on the controller with `show crypto pki servercert`, then click **Trust this certificate**. It is then pinned for the session, and a *different* certificate later is refused. For headless runs set `AOS8_CERT_FINGERPRINT`; if you deployed your own CA use `AOS8_CA_BUNDLE`. `AOS8_DEV_MODE` disables verification entirely and accepts any certificate — lab only. |
| `Certificate does not match the pinned fingerprint` | The controller presented a different certificate than the one you trusted. Either its certificate was legitimately replaced (re-verify and re-trust), or the connection is being intercepted. Do not click past this without checking. |
