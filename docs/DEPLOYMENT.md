...
| `AOS8_CA_BUNDLE` | _(unset — verification off)_ | Path to a CA bundle for verifying the AOS 8 controller's TLS certificate. Controllers ship self-signed certs, so verification is skipped by default; set this if you deployed your own CA — it protects the controller admin credentials from MITM on the management network |
