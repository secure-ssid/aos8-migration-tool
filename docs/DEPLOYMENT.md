...
| `AOS8_CA_BUNDLE` | _(unset — verification on)_ | Path to a CA bundle for verifying the AOS 8 controller's TLS certificate. Use this if you deployed your own CA to the controller. A stock factory self-signed cert does **not** need this — Step 1 shows its fingerprint and offers to pin it |
| `AOS8_CERT_FINGERPRINT` | _(unset)_ | Pin the controller's certificate by SHA-256 fingerprint (colons optional). The headless equivalent of the Step 1 trust prompt, for Docker/CI where nobody can click. Read it with `show crypto pki servercert`. Takes precedence over `AOS8_CA_BUNDLE` and `AOS8_DEV_MODE` |
| `AOS8_DEV_MODE` | _(unset — off)_ | `true` = local dev/test opt-out: permits cleartext `http://` destination base URLs, non-allowlisted destination hosts, and disables controller TLS verification **entirely** (accepts any certificate). For a self-signed controller prefer `AOS8_CERT_FINGERPRINT`, which is equally convenient and still rejects interception. Without dev mode, destination base URLs must be HTTPS on an HPE/Aruba host (`arubanetworks.com`, `cloud.hpe.com`, `greenlake.hpe.com`; loopback excepted) — a mistyped URL can never ship a bearer token to an unintended or cleartext endpoint. **Never set in production** |
